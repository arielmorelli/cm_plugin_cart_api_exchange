...
class TestExchangeApi(unittest.TestCase):
    def test_send_items(self):
        ...
        exchange_api._make_request = MagicMock()

        # empty items
        items = {}
        exchange_api.send_items("a-url", items, "cart-name")
        exchange_api._make_request.assert_not_called()

        # 1 item
        url = "a-url"
        cart_name = "a-simple-cart"
        copies = 10
        exchange_api._make_request.reset_mock()
        items = {1161: {"identifier": "1231231231231", "copies": copies}}

        exchange_api.send_items(url, items, cart_name)
        
        assert exchange_api._make_request.call_count == 1
        called_url, called_data = exchange_api._make_request.call_args[0]   
        assert called_url == url
        assert called_data["name"] == cart_name
        assert called_data["copies"] == copies
        ...
        assert exchange_api._make_request.call_count == 2
